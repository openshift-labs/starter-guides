...
In order to access a pod's terminal via the Web Console, click on *Workloads -> Pods* and finally the pod name as shown in the following image:
...
Before proceeding, close the connection to the pod from your local command line.
...
In order to show just the JAR file for the `parksmap` application, run the following:
