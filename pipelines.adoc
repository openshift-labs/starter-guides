...
button, find the `jenkins-ephemeral` template, and click on it.
...
image::pipeline-jenkins-params.png[Jenkins Template]
...
Click on the Jenkins route in order to open Jenkins Console. Login in Jenkins
using the admin credentials and explore the Jenkins Console.

NOTE: The admin credentials for Jenkins are: admin/openshift3

image::pipeline-jenkins-console.png[Jenkins Console]

Essentially, the OpenShift Jenkins plugin uses the OpenShift REST API in order
to integrate into various OpenShift operations and therefore you should grant
permission to the Jenkins service account (created above automatically) to
invoke these APIs. Run the following CLI command to allow Jenkins to retrieve
...
----
http://gitlab-ce-workshop-infra.{{ROUTER_ADDRESS}}/{{GITLAB_USER}}/nationalparks/raw/1.0.0/ose3/application-live.properties
----
