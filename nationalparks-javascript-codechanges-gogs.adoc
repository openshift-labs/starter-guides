...
external source via HTTP(S) when a change in a code repository happens.
...
In this lab, you can use a build webhook to trigger a pipeline execution every time there is a change in the nationalparks Gogs repository. In the OpenShift web console, navigate to your `{{ USER_PROJECT }}` project and then click *Builds -> Build Configs* in the left navigation. Click `nationalparks-build` build and scroll down to the Webhooks section.
...
On the next screen, paste your link into the "URL" field. You can leave the
...
image::nationalparks-codechanges-gogs-copy-secret.png[Secret]
