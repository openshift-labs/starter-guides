...
In this lab you can use a build webhook to trigger a pipeline execution every time there is a change in the nationalparks Gogs repository. In the OpenShift web console, navigate to your `{{ USER_PROJECT }}` project, and then mouse-over `Builds` and then `Pipelines`. Click the `nationalparks-build` build and then `Configuration` tab.
...
Once you have committed your changes, a *Pipeline Build* should almost instantaneously be
triggered in OpenShift. Look at the *Pipelines* page in the web console,
