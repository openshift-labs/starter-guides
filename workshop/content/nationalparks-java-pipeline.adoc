...
In the *userXY* project, click on *Workloads -> Deployment Configs* on the left sidebar menu
and then on *nationalparks* deployment config. Click on *Actions -> Edit Deployment Config*:
...
In the Developer Catalog, search for *Jenkins*. Select the one named *Jenkins*.

image::images/devops-pipeline-catalog-search.png[Empty CICD project]

We will be prompted with the configuration screens for deploying our Jenkins server instance. Click the blue *Create Service Instance* button to get started.

image::images/devops-jenkins-template.png[Jenkins template]

Leave the defaults for all parameters except *Memory Limit*, which should be set to `1Gi`. Then click the blue *Create* button.

image::images/devops-jenkins-template-config.png[Jenkins template config]

Jenkins will now be provisioned on the background. Click *Catalog -> Provisioned Services* in the left navigation to see your Jenkins service. It may take a couple minutes for the provisioning to complete.
...
Now click *Home -> Status* and you should see the Jenkins deployment on your Project Status page.

image::images/devops-jenkins-project-status.png[Jenkins deployment]

Notice how you have the actual Jenkins deployment and a Provisioned Service, this is because we deployed Jenkins from the Developer Catalog as a service for our current project.
...
pipeline {
  ...
  stages {
    stage('Build JAR') {
      steps {
        git url: 'http://gogs-{{INFRA_PROJECT}}.{{ROUTER_ADDRESS}}/%username%/nationalparks'
...
In the **userXY** project, click on *Add -> Import YAML* and paste the following YAML file to create a pipeline 
...
spec:
  ...
  source:
    git:
      ref: master
      uri: "http://gogs-{{INFRA_PROJECT}}.{{ROUTER_ADDRESS}}/%username%/nationalparks"
    type: Git
  strategy:
...
In OpenShift Web Console, click on *Builds -> Builds* and then **nationalparks-build-1** to see the pipeline you just created.
