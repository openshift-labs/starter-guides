...
In order to start the Pipeline, we can use a `PipelineRun` Custom Resource, representing our parameter to send to the pipeline:
...
Click the *Pipeline* tab, then *nationalparks-build-1* to see the pipeline you just created.
