In this lab, we're going to deploy a backend service, developed in Java that will expose 2 main REST endpoints to the visualizer
...
CAUTION: In the following step(s), replace `{{ project_namespace  }}` with the project provided to you.
...
This is the developer catalog which allows a user to select components they want to add to their application.  In this case, we are using Spring Boot so we want to select a JDK without an application server.

Search for `openjdk`, and click *Red Hat OpenJDK 8* as shown in the following image:

image::images/nationalparks-java-sarch-jdk.png[Developer Catalog Languages]

After you click *Red Hat OpenJDK 8*, a dialog is presented as shown in the following image:

image::images/nationalparks-java-new-java-service.png[Create application]
...
image::images/nationalparks-java-configure-java-service.png[Runtimes]
...
nationalparks   nationalparks-{{ project_namespace  }}.{{ROUTER_ADDRESS}}             nationalparks   8080-tcp
parksmap        parksmap-{{ project_namespace  }}.{{ROUTER_ADDRESS}}                  parksmap        8080-tcp
----
...
[source,bash,role=copypaste]
----
http://nationalparks-{{ project_namespace  }}.{{ROUTER_ADDRESS}}
----
