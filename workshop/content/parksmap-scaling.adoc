...
You can also scale up to two pods in the Developer Console. From the Topology view, first click the `parksmap` deployment config and select the *Overview* tab:

image::images/parksmap-scaleup.png[Scaling]

Next, click the *^* icon next to the Pod visualization to scale up to 2 pods.

image::images/parksmap-scaled.png[Scaled Up]
...
You can also see that both *Pods* are running in the Developer Console:
