...
In the *Administrator* view, click *Projects* in the left navigation if it isn't already selected. Find your *{{ USER_PROJECT }}* project and click it. 

image::images/parksmap-administrator-projects.png[Select Project]

On the next page, you'll see some details about the resources in your project. Since the project is currently empty, there isn't much to see here for now. Click *Administrator* to switch to the *Developer* console.

image::images/parksmap-administrator-project-details.png[Project Details]

This will take you to the *Topology* view in the Developer Console. Click *Container Image* to open a dialog that will allow you to specify the information for the image you want to deploy.  
...
presented with some options to add labels, scaling, etc. -- which we will learn about later.
...
If you click on the `parksmap` entry in the Topology view, you will see some information about that deployment config. On that panel, you will see that there is a single *Pod* that was created by your actions. 

image::images/parksmap-overview.png[Pod overview]

You can also get a list of all the *Pods* created within your *Project*, by navigating to *Workloads -> Pods* in the *Administrator* console.
...
In the Developer Console from the Topology view, service information is available by clicking the `parksmap` deployment config, then *Resources*, and then you should see the `parksmap` entry in the *Services* section.
