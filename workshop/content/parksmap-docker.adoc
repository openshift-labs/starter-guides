...
Docker-formatted image to run on OpenShift. Docker container images are lightweight, standalone, executable packages of software that include everything needed to run an application.
Launching such containers is incredible easy with OpenShift, and it can be done directly from the web console.
...
Either press enter or click on the magnifying glass. OpenShift will then go out to the container registry specified and interrogate the image.
...
For *Resources*, ensure Deployment Config is selected to automatically manage the ReplicationController (RC), which we'll learn about soon.
