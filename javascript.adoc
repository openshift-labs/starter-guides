...
https://github.com/openshift-roadshow/nationalparks-js[https://github.com/openshift-roadshow/nationalparks-js]
...
http://{{GITLAB_URL_PREFIX}}.{{ROUTER_ADDRESS}}/{{GITLAB_USER}}/nationalparks-js/tree/{{NATIONALPARKS_VERSION}}[http://{{GITLAB_URL_PREFIX}}.{{ROUTER_ADDRESS}}/{{GITLAB_USER}}/nationalparks-js/tree/{{NATIONALPARKS_VERSION}}]
...
let's use it with a simple Javascript S2I image.
{% else %} Since for this lab you have your own GitLab repository let's use it
with a simple Javascript S2I image.
{% endif %}
...
Use the Service Catalog to select *"Languages"*, then *"JavaScript"*, then *"Node.js"*

image::service_catalog_js.png[Runtimes]

After confirming your selection of `nodejs`, you will need to enter a
...
----
{% if GIT_HOST == "github" %}
https://github.com/(your Github username)/nationalparks-js.git
{% else %}
http://{{GITLAB_URL_PREFIX}}.{{ROUTER_ADDRESS}}/{{GITLAB_USER}}/nationalparks-js.git
{% endif %}
----
...
image::service_catalog_js_confirm.png[Runtimes]
...
But we don't have a database. Yet&hellip;
