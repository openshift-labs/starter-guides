...
Find your *{{ USER_PROJECT }}* project and click it. On the next page, click *"Add to Project"* and a drop down menu will appear. There are several options, but we are only concerned with "Deploy Image". Click "Deploy Image" in the *"Add to Project"* drop down.  

image::parksmap-add-to-project-dialog.png[Add to Project]

This will open a dialog that will allow you to specify the information for the image you want to deploy. We will learn more about image streams and image stream tags later. For now,
...
"Continue to the project overview." link or the "Close" button. Take a moment to look at the various messages that
